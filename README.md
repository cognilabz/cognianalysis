...
cognianalysis dev run-orchestration . # validate Codex-authored source-tier workpack receipts and record worker/cache logs
cognianalysis dev prove-orchestration . # materialize parallel/cache proof from harness execution/cache logs
...
`cognianalysis dev run-orchestration` records harness-owned `.analysis/data/orchestration-execution-log.json` and `.analysis/data/cache-ledger.json` only after each completed source-tier output carries a valid Codex-authored workpack execution receipt bound to the task manifest, reviewed source paths and review hash. The runner validates those receipts concurrently and never reads `.analysis-seed` as orchestration input. `cognianalysis dev prove-orchestration` then writes `.analysis/data/parallel-execution-proof.json` and `.analysis/data/cache-reuse-proof.json` only from those runner-generated logs: the execution log must record at least two source-tier workpack worker executions with matching task IDs, timings, output hashes and receipt hashes, and the cache ledger must record a prior cache entry reused later with a deterministic cache key. Small one-workpack analyses, post-hoc completed outputs without Codex workpack receipts, post-hoc outputs without runner-generated logs, and cache-key generation without prior reuse are rejected instead of being self-certified as parallel execution.
