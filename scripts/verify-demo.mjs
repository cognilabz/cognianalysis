...
const doctorMarketStrictOutput = run(['dev', 'doctor', demo, '--market-proof', '--strict'], { capture: true }).stdout || '';
assert(doctorMarketStrictOutput.includes('Strict market proof: ready'), 'doctor --market-proof --strict must pass after representative golden and baseline proof exists');
...
assert(evalOutput.includes('Verdict: NOT_PRODUCT_READY') || evalOutput.includes('Verdict: PARTIALLY_READY'), 'eval must not claim full product readiness for the single demo suite');
assert(evalOutput.includes('Core ideas covered:'), 'eval must answer whether original ideas are covered');
assert(evalOutput.includes('Core features implemented:'), 'eval must answer whether original features are implemented');
...
assert(!evalOutput.includes('PRODUCT-MISSING multi_repo_benchmark'), 'eval must not keep claiming representative golden proof is missing after five suites pass');
assert(!evalOutput.includes('PRODUCT-MISSING baseline_comparison'), 'eval must not keep claiming baseline proof is missing after required baselines pass');
...
assert(evalOutput.includes('PRODUCT-MISSING thin_artifact_model') || evalOutput.includes('PRODUCT-MISSING parallel_orchestration'), 'eval must still identify the remaining simplification/orchestration gap');
...
assert(helpOutput.includes('--mode brief|blueprint|deep|complete-audit'), 'CLI help must expose v0.8 product analysis modes');
assert(helpOutput.includes('--goal text'), 'CLI help must expose goal-first product input');
assert(helpOutput.includes('--flow name') && helpOutput.includes('--module path') && helpOutput.includes('--api name'), 'CLI help must expose deep target flags');
assert(!helpOutput.includes('dev resume [repo]'), 'Public CLI help must not expose dev resume');
assert(!helpOutput.includes('dev prepare [repo]'), 'Public CLI help must not expose internal prepare');
...
assert(!helpOutput.includes('dev repair [repo]'), 'Public CLI help must not expose dev repair');
assert(helpOutput.includes('open [repo]'), 'CLI help must expose product-mode open');
const devHelpOutput = run(['dev', '--help'], { capture: true }).stdout || '';
assert(devHelpOutput.includes('dev resume [repo]'), 'Dev help must expose resume recovery under dev namespace');
assert(devHelpOutput.includes('dev prepare [repo]'), 'Dev help must expose internal prepare under dev namespace');
assert(devHelpOutput.includes('dev repair [repo]'), 'Dev help must expose repair recovery under dev namespace');
...
assert(bundle.product_artifact_model?.complete === true, `Demo thin artifact model incomplete: ${(bundle.product_artifact_model?.missing || []).join(', ')}`);
...
writeJson(join(orchestrationAnalysis, 'cache', 'source-tier', 'wrong-cache-store-name.json'), firstCacheStore);
run(['dev', 'aggregate', orchestrationRepo], { capture: true });
orchestrationBundle = JSON.parse(readFileSync(join(orchestrationAnalysis, 'data', 'bundle.json'), 'utf8'));
assert(orchestrationBundle.parallel_orchestration_contract?.complete === false, 'Wrong cache-store filenames must not complete orchestration');
assert(orchestrationBundle.parallel_orchestration_contract?.cache_reuse_proof_validation?.cache_ledger_validation?.missing?.includes('cache_ledger.cache_store_files'), 'Cache ledger hits must bind to the exact runner-created cache-store filename');
rmSync(join(orchestrationAnalysis, 'cache', 'source-tier', 'wrong-cache-store-name.json'), { force: true });
writeJson(firstCacheStorePath, firstCacheStore);
const mismatchedCacheStore = { ...firstCacheStore, created_at: '2026-01-01T00:00:00.000Z' };
writeJson(firstCacheStorePath, mismatchedCacheStore);
run(['dev', 'aggregate', orchestrationRepo], { capture: true });
orchestrationBundle = JSON.parse(readFileSync(join(orchestrationAnalysis, 'data', 'bundle.json'), 'utf8'));
assert(orchestrationBundle.parallel_orchestration_contract?.complete === false, 'Mismatched cache-store timestamps must not complete orchestration');
assert(orchestrationBundle.parallel_orchestration_contract?.cache_reuse_proof_validation?.cache_ledger_validation?.missing?.includes('cache_ledger.cache_store_files'), 'Cache ledger hits must bind to cache-store created_at');
writeJson(firstCacheStorePath, firstCacheStore);
...
try {
  ...
  const deepDive = run(['analyze', tempRepo, '--mode', 'deep', '--flow', 'onboarding', '--no-seed', '--no-html'], { capture: true });
  assert((deepDive.stdout || '').includes('Cognianalysis analyze: mode=deep'), 'Analyze must accept a targeted deep mode');
  const deepDiveRequest = JSON.parse(readFileSync(join(tempRepo, '.analysis', 'data', 'product-analysis-request.json'), 'utf8'));
  assert(deepDiveRequest.target?.flow === 'onboarding', 'Deep analyze must persist the requested flow target');
  assert(deepDiveRequest.analysis_scope_request?.mode === 'representative', 'Deep must preserve an existing explicit/non-complete scope on an existing request');
  rmSync(join(tempRepo, '.analysis'), { recursive: true, force: true });
  const missingTarget = run(['analyze', tempRepo, '--mode', 'deep', '--no-seed', '--no-html'], { capture: true, expectFailure: true });
  assert(String(missingTarget.stderr || missingTarget.stdout || '').includes('Deep mode requires --goal'), 'Deep without a target or goal must fail closed');
  ...
  rmSync(join(tempRepo, '.analysis'), { recursive: true, force: true });
  const completeScopeFilesOnly = run(['analyze', tempRepo, '--mode', 'complete-audit', '--scope-files', '5', '--no-seed', '--no-html'], { capture: true, expectFailure: true });
  assert(String(completeScopeFilesOnly.stderr || completeScopeFilesOnly.stdout || '').includes('--scope-files requires --scope'), 'Complete-audit mode with --scope-files but no --scope must fail clearly');
  ...
  const changedRequest = run(['analyze', readyRequestRepo, '--mode', 'deep', '--flow', 'onboarding'], { capture: true, expectFailure: true });
  ...
  run(['analyze', equalMtimeRepo, '--mode', 'deep', '--flow', 'onboarding'], { capture: true, expectFailure: true });
