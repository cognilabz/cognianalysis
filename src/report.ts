import { Path, ensureDir, escapeHtml, loadJson, safeJsonForHtml, writeJson, writeText } from './utils';
...
export function renderReport(analysisDir: string, outputDir?: string, title?: string): string {
  ...
  writeJson(Path.join(out, 'analysis-data.json'), bundle);
  return index;
}
...
function chips(values: any, empty = 'No data'): string {
  if (!Array.isArray(values)) values = values === undefined || values === null ? [] : [values];
  return values.slice(0, 28).map((v: any) => chip(v)).join('') || `<span class="muted small">${escapeHtml(empty)}</span>`;
}
...
function pre(value: any, cls = ''): string {
...
function evidenceHtml(items: any): string {
  if (!Array.isArray(items) || !items.length) return '<span class="muted small">no evidence</span>';
...
function listItems(items: any[], render: (x: any, i: number) => string, empty = 'No extracted data yet.'): string {
  if (!Array.isArray(items) || !items.length) return `<div class="empty">${escapeHtml(empty)}</div>`;
  return items.map(render).join('');
}

function sectionOverview(bundle: any): string {
  const p = bundle.profile || {};
  const s = bundle.status || {};
  const ev = bundle.evidence_index || [];
  const sc = bundle.source_coverage || {};
  const invalid = ev.filter((e: any) => e.valid === false).length;
  ...
  <div class="grid two">
    ${card('Repository Profile', `<div class="kv"><span>Name</span><strong>${escapeHtml(p.repo_name)}</strong></div><div class="kv"><span>Type</span><strong>${escapeHtml(p.repo_type)}</strong></div><div class="kv"><span>Frameworks</span><div>${chips(p.frameworks)}</div></div><div class="kv"><span>Build tools</span><div>${chips(p.build_tools)}</div></div><div class="kv"><span>Commit</span><code>${escapeHtml(p.commit || 'n/a')}</code></div>`)}
...
function sectionSourceCoverage(bundle: any): string {
  const sc = bundle.source_coverage || {};
  const modules = sc.modules || [];
  const uncovered = sc.uncovered || [];
  const skipped = sc.skipped || [];
  const rows = modules.map((m: any) => `<tr class="search-card" data-search="${escapeHtml(`${m.name} ${m.uncovered_files}`)}"><td><strong>${escapeHtml(m.name)}</strong></td><td>${escapeHtml(m.files || 0)}</td><td>${escapeHtml(m.covered_files || 0)}</td><td>${escapeHtml(m.uncovered_files || 0)}</td><td>${chip((m.uncovered_files || 0) === 0 ? 'complete' : 'partial', (m.uncovered_files || 0) === 0 ? 'ok' : 'warn')}</td></tr>`).join('');
  return `<div class="metrics">
  ...
  <div class="grid two">
    ${card('Uncovered Files', listItems(uncovered, (f: any) => `<div class="subitem"><strong>${escapeHtml(f.path)}</strong><div>${chips(f.roles || [])}</div><p class="muted small">${escapeHtml(f.module || '')} · ${escapeHtml(f.language || '')} · ${escapeHtml(f.lines || 0)} lines</p></div>`, 'No uncovered files.'))}
    ${card('Skipped Files', listItems(skipped, (f: any) => `<div class="subitem"><strong>${escapeHtml(f.path)}</strong><p class="muted small">${escapeHtml(f.reason || '')} · ${escapeHtml(f.bytes || 0)} bytes</p></div>`, 'No skipped files.'))}
  </div>`;
}

function sectionTargetCoverage(bundle: any): string {
  const rows = (bundle.target_coverage || []).map((c: any) => {
    const statusCls = c.output_status === 'present' ? 'ok' : c.output_status === 'partial' ? 'warn' : 'pending';
    const details = (c.output_details || []).map((d: any) => `${d.present ? '✓' : '○'} ${d.key}`).join(' · ');
    return `<tr class="search-card" data-search="${escapeHtml(c.title + ' ' + c.description + ' ' + details)}"><td><strong>${escapeHtml(c.title)}</strong><div class="small muted">${escapeHtml(c.id)}</div></td><td>${chip(c.design_status, 'ok')}</td><td>${chip(c.output_status, statusCls)}</td><td>${escapeHtml(c.description)}</td><td><div class="small">${escapeHtml(details)}</div><div>${chips(c.addressed_by || [])}</div></td></tr>`;
  }).join('');
...
function sectionAssessment(bundle: any): string {
  ...
  const risks = listItems(a.top_risks || [], (r: any) => `<li><strong>${escapeHtml(r.title || 'Risk')}</strong> ${chip(r.severity || 'medium')}<br><span class="muted">${escapeHtml(r.description || '')}</span>${evidenceHtml(r.evidence)}</li>`);
  const steps = listItems(a.recommended_next_steps || [], (x: any) => `<li><strong>${escapeHtml(x.title || 'Step')}</strong><br><span class="muted">${escapeHtml(x.reason || '')}</span>${evidenceHtml(x.evidence)}</li>`);
  return `<div class="grid two">${card('Executive Summary', `<p>${escapeHtml(a.executive_summary || '')}</p><h4>System Purpose</h4><p>${escapeHtml(a.system_purpose || '')}</p>`, 'accent')}${card('Key Scope', `<div class="kv"><span>Capabilities</span><div>${chips(a.key_capabilities)}</div></div><div class="kv"><span>Interfaces</span><div>${chips(a.key_interfaces)}</div></div><div class="kv"><span>Scope</span><div>${chips(a.assessment_scope)}</div></div>`)}</div><div class="metrics">${comp}</div><div class="grid two">${card('Top Risks', `<ul>${risks}</ul>`)}${card('Recommended Next Steps', `<ul>${steps}</ul>`)}</div>`;
}

function sectionDecisionBasis(bundle: any): string {
  const a = bundle.assessment || {};
  const d = a.decision_basis || {};
  const t = a.tool_positioning || {};
  return `<div class="grid two">${card('Decision Summary', `<p>${escapeHtml(d.decision_summary || '')}</p><h4>Readiness</h4>${pre(d.readiness)}${evidenceHtml(d.evidence)}`, 'accent')}${card('Tool Positioning', `<p>${escapeHtml(t.summary || '')}</p><div>${chip(t.automation_level || 'automation n/a')}</div><h4>Strengths</h4>${pre(t.strengths_vs_traditional_tools || [])}<h4>Boundaries</h4>${pre(t.boundaries || [])}${evidenceHtml(t.evidence)}`)}</div><div class="grid two">${card('Recommended Actions', listItems(d.recommended_actions || [], (x: any) => `<div class="subitem"><strong>${escapeHtml(x.title || 'Action')}</strong> ${chip(x.priority || 'priority n/a')}<p>${escapeHtml(x.rationale || '')}</p>${evidenceHtml(x.evidence)}</div>`, 'No decision actions extracted.'))}${card('Trade-offs', listItems(d.tradeoffs || [], (x: any) => `<div class="subitem"><strong>${escapeHtml(x.topic || 'Trade-off')}</strong><h4>Options</h4>${pre(x.options || [])}<p>${escapeHtml(x.recommendation || '')}</p>${evidenceHtml(x.evidence)}</div>`, 'No trade-offs extracted.'))}</div>`;
}

function sectionFunctionalTechnical(bundle: any): string {
  const a = bundle.assessment || {};
  const f = a.functional_view || {};
  const t = a.technical_view || {};
  return `<div class="grid two">${card('Functional View', `<p>${escapeHtml(f.summary || '')}</p><h4>Actors</h4>${chips(f.actors || [])}<h4>Capabilities</h4>${chips(f.capabilities || [])}<h4>User/System Flows</h4>${listItems(f.user_or_system_flows || [], (x: any) => `<div class="subitem"><strong>${escapeHtml(x.name || 'Flow')}</strong><p>${escapeHtml(x.description || '')}</p>${evidenceHtml(x.evidence)}</div>`, 'No functional flows extracted.')}${evidenceHtml(f.evidence)}`, 'accent')}${card('Technical View', `<p>${escapeHtml(t.summary || '')}</p><h4>APIs / Interfaces</h4>${chips(t.apis || [])}<h4>Architecture</h4>${chips(t.architecture || [])}<h4>Data / Integrations</h4>${chips(t.data_and_integrations || [])}${evidenceHtml(t.evidence)}`, 'accent')}</div>`;
}

function sectionBusiness(bundle: any): string {
  const caps = bundle.capabilities || [];
  const logic = bundle.business_logic || [];
  const capHtml = listItems(caps, (c: any) => card(c.name || c.id || 'Capability', `<p>${escapeHtml(c.description || '')}</p><div>${chips(c.actors || [])}${chips(c.domain_terms || [])}</div><h4>Business rules</h4>${listItems(c.business_rules || [], (r: any) => `<div class="subitem"><strong>${escapeHtml(r.rule_type || 'rule')}</strong>: ${escapeHtml(r.description || '')}${evidenceHtml(r.evidence)}</div>`, 'No rules extracted.') }<h4>Business logic examples</h4>${listItems(c.business_logic || [], (l: any) => `<div class="subitem"><strong>${escapeHtml(l.name || l.title || 'Logic')}</strong><p>${escapeHtml(l.description || '')}</p>${pre(l.example)}${evidenceHtml(l.evidence)}</div>`, 'No business logic examples extracted.') }<h4>Function/use-case examples</h4>${listItems(c.function_examples || [], (e: any) => `<div class="subitem"><strong>${escapeHtml(e.title || e.function_or_use_case || 'Example')}</strong>${pre({ input: e.input, output: e.output, explanation: e.explanation, example_origin: e.example_origin })}${evidenceHtml(e.evidence)}</div>`, 'No function examples extracted.')}${evidenceHtml(c.evidence)}`));
  const logicHtml = listItems(logic, (l: any) => card(l.title || l.id || 'Business Logic', `<p>${escapeHtml(l.description || '')}</p>${pre(l.examples)}${evidenceHtml(l.evidence)}`));
  return `<div class="grid two"><div>${capHtml}</div><div>${logicHtml}</div></div>`;
}

function sectionInterfaces(bundle: any): string {
  const items = bundle.interfaces || [];
  return listItems(items, (it: any) => card(it.name || it.id || 'Interface', `<div>${chip(it.type || 'interface')}${chip(it.protocol || '')}${chip(it.method || '')}${chip(it.path || '')}</div><p>${escapeHtml(it.description || '')}</p><div class="grid two"><div><h4>Request</h4>${pre(it.request)}</div><div><h4>Response</h4>${pre(it.response)}</div></div><h4>Examples</h4>${listItems(it.examples || [], (ex: any) => `<div class="subitem"><strong>${escapeHtml(ex.title || 'Example')}</strong> ${chip(ex.example_origin || 'unknown')}${pre({ request: ex.request, response: ex.response })}${evidenceHtml(ex.evidence)}</div>`, 'No examples extracted.') }<h4>Errors / faults</h4>${pre(it.errors)}<h4>Source contracts</h4>${pre(it.source_contracts)}${evidenceHtml(it.evidence)}`));
}

function docList(documentation: any, key: string): any[] {
...
function sectionExamples(bundle: any): string {
...
function sectionFlows(bundle: any): string {
  const flows = bundle.flows || [];
  const docFlows = docList(bundle.documentation || {}, 'mermaid_flows');
  const renderFlow = (f: any) => card(f.title || f.id || 'Flow', `<p>${escapeHtml(f.summary || '')}</p>${f.mermaid ? `<div>${chip(f.mermaid.diagram_type || 'mermaid')}</div>${pre(f.mermaid.source, 'mermaid')}${evidenceHtml(f.mermaid.evidence)}` : ''}${f.source ? `<div>${chip(f.diagram_type || 'mermaid')}</div>${pre(f.source, 'mermaid')}` : ''}<h4>Steps</h4>${listItems(f.steps || [], (s: any) => `<div class="step"><span>${escapeHtml(s.order || '')}</span><div><strong>${escapeHtml(s.kind || s.actor || 'step')}</strong><p>${escapeHtml(s.description || '')}</p>${evidenceHtml(s.evidence)}</div></div>`, 'No steps extracted.') }<h4>Side effects</h4>${pre(f.side_effects)}<h4>Examples</h4>${pre(f.examples)}${evidenceHtml(f.evidence)}`);
  return `<h3>Extracted Flows</h3>${listItems(flows, renderFlow)}<h3>Documentation Mermaid Flows</h3>${listItems(docFlows, renderFlow)}`;
}

function sectionDomain(bundle: any): string {
  const dm = bundle.domain_model || {};
  const data = bundle.data_model || {};
  return `<div class="grid two">${card('Domain Glossary', listItems(dm.glossary || [], (g: any) => `<div class="subitem"><strong>${escapeHtml(g.term)}</strong><p>${escapeHtml(g.meaning || '')}</p>${evidenceHtml(g.evidence)}</div>`, 'No glossary extracted.'))}${card('Domain Entities', listItems(dm.entities || [], (e: any) => `<div class="subitem"><strong>${escapeHtml(e.name)}</strong><p>${escapeHtml(e.description || '')}</p>${pre(e.key_fields || e.fields)}${evidenceHtml(e.evidence)}</div>`, 'No domain entities extracted.'))}</div><div class="grid two">${card('Data Entities and Stores', `<h4>Entities</h4>${pre(data.entities)}<h4>Stores</h4>${pre(data.stores)}<h4>State changes</h4>${pre(data.state_changes)}`)}${card('Integrations and Side Effects', `<h4>Integrations</h4>${pre(bundle.integrations || [])}<h4>Side effects</h4>${pre(bundle.side_effects || [])}`)}</div>`;
}

function sectionArchitecture(bundle: any): string {
  const a = bundle.architecture || {};
  const target = a.target_architecture || {};
  return `<div class="grid two">${card('Architecture Summary', `<p>${escapeHtml(a.summary || '')}</p><div>${chip(a.style || 'unknown')}</div>${a.mermaid ? pre(a.mermaid, 'mermaid') : ''}`)}${card('Target Architecture / Tech Stack', `<p>${escapeHtml(target.summary || '')}</p><div>${chip(target.target_style || 'target n/a')}</div><h4>Tech Stack Options</h4>${pre(target.tech_stack_options || [])}<h4>Migration Steps</h4>${pre(target.migration_steps || [])}<h4>Open Questions</h4>${pre(target.open_questions || [])}`)}</div><div class="grid two">${card('Modules', listItems(a.modules || [], (m: any) => `<div class="subitem"><strong>${escapeHtml(m.name)}</strong><p>${escapeHtml(m.responsibility || '')}</p>${chips(m.dependencies || [])}${evidenceHtml(m.evidence)}</div>`, 'No architecture modules extracted.'))}${card('External Systems', pre(a.external_systems || []))}</div><div class="grid two">${card('Runtime / Data Stores', `${pre(a.runtime || [])}${pre(a.data_stores || [])}`)}${card('Observations', listItems(a.observations || [], (o: any) => `<div class="subitem"><strong>${escapeHtml(o.title)}</strong><p>${escapeHtml(o.description || '')}</p>${evidenceHtml(o.evidence)}</div>`, 'No observations extracted.'))}</div>`;
}

function sectionProcess(bundle: any): string {
  ...
  return `<div class="card accent"><h3>Process Summary</h3><p>${escapeHtml(p.summary || '')}</p></div><div class="metrics">${processCards}</div><div class="grid two">${card('Strengths', pre(q.strengths || []))}${card('Risks', pre(q.risks || []))}</div><div class="grid two">${card('Security / Weaknesses', pre(q.security || []))}${card('Testability', pre(q.testability || []))}</div>`;
}

function sectionFindings(bundle: any): string {
  return listItems(bundle.findings || [], (f: any) => card(f.title || f.id || 'Finding', `<div>${chip(f.category || 'risk')}${chip(f.severity || 'medium')}</div><p>${escapeHtml(f.description || '')}</p><h4>Recommendation</h4><p>${escapeHtml(f.recommendation || '')}</p>${evidenceHtml(f.evidence)}`));
}

function sectionRefactoring(bundle: any): string {
  const all = [...(bundle.refactoring || []), ...(bundle.modernization || [])];
  return listItems(all, (r: any) => card(r.title || r.id || 'Roadmap Item', `<div>${chip(r.risk || 'risk n/a')}${chip(r.effort || 'effort n/a')}</div><p>${escapeHtml(r.description || '')}</p><h4>Benefit</h4><p>${escapeHtml(r.benefit || '')}</p><h4>Target state</h4><p>${escapeHtml(r.target_state || '')}</p><h4>Candidate files</h4>${chips(r.candidate_files || [])}${evidenceHtml(r.evidence)}`));
}

function sectionEvidence(bundle: any): string {
  const ev = bundle.evidence_index || [];
  return listItems(ev, (e: any) => `<div class="evidence-row search-card ${e.valid === false ? 'bad' : 'ok'}" data-search="${escapeHtml(`${e.path}:${e.line} ${e.symbol || ''} ${e.snippet || ''}`)}"><strong>${escapeHtml(e.path)}:${escapeHtml(e.line || 1)}</strong>${e.symbol ? ` <span class="muted">${escapeHtml(e.symbol)}</span>` : ''}${e.valid === false ? ` ${chip(e.reason || 'invalid', 'bad')}` : ''}${e.snippet ? `<pre>${escapeHtml(e.snippet)}</pre>` : ''}</div>`, 'No evidence collected yet.');
}
...
function sectionCodeMap(bundle: any): string {
  const modules = bundle.modules || [];
  const docs = bundle.important_docs || [];
  const capsules = bundle.capsules || [];
  const files = bundle.files || [];
  return `<div class="grid two">${card('Important Docs / Contracts / Examples', listItems(docs.slice(0, 60), (d: any) => `<div class="subitem"><strong>${escapeHtml(d.path)}</strong><div>${chips(d.roles || [])}</div>${pre(d.signals || [])}</div>`, 'No candidates found.'))}${card('Source Capsules', listItems(capsules.slice(0, 30), (c: any) => `<div class="subitem"><strong>${escapeHtml(c.path)}</strong><div>${chips(c.roles || [])}</div><p class="muted small">score ${escapeHtml(c.score)} · ${escapeHtml(c.lines)} lines</p></div>`, 'No capsules found.'))}</div><h3>Modules</h3><div class="table-wrap"><table><thead><tr><th>Module</th><th>Files</th><th>Source</th><th>Roles</th></tr></thead><tbody>${modules.slice(0, 80).map((m: any) => `<tr class="search-card" data-search="${escapeHtml(m.name + ' ' + JSON.stringify(m.roles || {}))}"><td><strong>${escapeHtml(m.name)}</strong></td><td>${escapeHtml(m.files)}</td><td>${escapeHtml(m.source_files)}</td><td>${chips(Object.keys(m.roles || {}))}</td></tr>`).join('')}</tbody></table></div><h3>Ranked Files</h3><div class="table-wrap"><table><thead><tr><th>File</th><th>Roles</th><th>Signals</th><th>Symbols</th><th>Score</th></tr></thead><tbody>${files.slice(0, 120).map((f: any) => `<tr class="search-card" data-search="${escapeHtml(f.path + ' ' + (f.roles || []).join(' '))}"><td><code>${escapeHtml(f.path)}</code></td><td>${chips(f.roles || [])}</td><td>${escapeHtml(f.signal_count || 0)}</td><td>${escapeHtml(f.symbol_count || 0)}</td><td>${escapeHtml(f.score || 0)}</td></tr>`).join('')}</tbody></table></div>`;
}
...
<div class="layout">
<aside class="sidebar"><div class="brand"><div class="logo">CU</div><div><div class="brand-title">Codebase Understanding</div><div class="brand-subtitle">LLM-first · TypeScript · evidence</div></div></div><nav>${nav}</nav><div class="side-note"><strong>${escapeHtml(bundle.profile?.repo_name || 'Repository')}</strong><br><span>${escapeHtml(bundle.profile?.repo_type || 'unknown')}</span></div></aside>
<main><header class="hero"><div><p class="eyebrow">Interactive HTML Report</p><h1>${escapeHtml(title)}</h1><p class="muted">${escapeHtml(bundle.profile?.root || '')}</p></div><div class="actions"><input id="search" type="search" placeholder="Search: API, request, Mermaid, rule, file …"><button id="theme" type="button">Theme</button></div></header>${htmlSections}</main>
</div>
<script>${JS}</script>
</body>
</html>`;
