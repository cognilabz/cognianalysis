...
export function writeLlmTasks(analysisDir: string, codeMap: CodeMap): any[] {
  ...
  writeText(Path.join(analysisDir, 'TASK.md'), singleTaskGuide(profile, tierManifest, taskDefs, templateDefs));
  writeJson(Path.join(analysisDir, 'task-manifest.json'), { mode: 'llm_first_workflow_tasks', implementation_language: 'TypeScript', single_task_file: 'TASK.md', pipeline, source_tier_tasks: tierManifest.tasks, capability_templates: templateDefs, tasks: taskDefs });
  return taskDefs;
}

function singleTaskGuide(profile: any, tierManifest: any, taskDefs: any[], templateDefs: any[]): string {
