...
function goldenHasEvidence(item: any): boolean {
  return asList(item?.evidence).length > 0 || asList(item?.evidence_refs).length > 0;
}

function addGoldenReportFact(out: any[], text: any, item: any, source: string): void {
  const value = String(text || '').trim();
  if (!value) return;
  out.push({ text: value, source, has_evidence: goldenHasEvidence(item) });
}

function goldenReportFacts(bundle: any): any[] {
  const out: any[] = [];
  for (const section of asList(bundle?.analysis_document?.sections)) {
    for (const block of asList(section?.blocks)) {
      ...
      for (const item of asList(block?.entries)) addGoldenReportFact(out, item.name || item.title, item, `${section.id || section.title}:${type}:entry`);
      for (const item of asList(block?.exits)) addGoldenReportFact(out, item.name || item.title, item, `${section.id || section.title}:${type}:exit`);
      for (const item of asList(block?.state)) addGoldenReportFact(out, item.name || item.title, item, `${section.id || section.title}:${type}:state`);
      for (const item of asList(block?.steps)) addGoldenReportFact(out, item.description || item.title, item, `${section.id || section.title}:${type}:step`);
      for (const item of asList(block?.items)) addGoldenReportFact(out, item.title || item.name || item.description, item, `${section.id || section.title}:${type}:item`);
      ...
      for (const item of asList(block?.levels)) addGoldenReportFact(out, item.level || item.title, item, `${section.id || section.title}:${type}:level`);
      for (const item of asList(block?.families)) addGoldenReportFact(out, item.name || item.title, item, `${section.id || section.title}:${type}:family`);
    }
  }
  return out;
}

function goldenRepresentativeCoverage(root: string, expectedFiles: string[], suiteRows: any[]): any {
  const manifestFile = Path.join(root, 'benchmarks', 'golden', 'manifest.json');
  const manifest = loadJson<any | null>(manifestFile, null);
  const expectedByFile = new Map(expectedFiles.map(file => {
    const expectedFile = posixRelative(root, file);
    return [expectedFile, loadJson<any>(file, {})];
  }));
  const suiteByExpectedFile = new Map(suiteRows.map((suite: any) => [String(suite.expected_file || ''), suite]));
  const errors: string[] = [];
  if (!manifest) {
    return {
  ...
  for (const category of DEFAULT_REQUIRED_GOLDEN_CATEGORIES) {
    if (!requiredCategories.includes(category)) errors.push(`manifest required_categories missing ${category}`);
  }

  const seenExpectedFiles = new Set<string>();
  const seenRepos = new Set<string>();
  const duplicateRepos = new Set<string>();
  const rows = asList(manifest.suites).map((row: any, index: number) => {
    ...
    if (!expected_file) rowErrors.push('expected_file is required');
    if (expected_file && !expectedByFile.has(expected_file)) rowErrors.push('expected_file does not match a discovered golden expected file');
    if (expected_file && seenExpectedFiles.has(expected_file)) rowErrors.push('expected_file is duplicated in manifest');
    if (expected_file) seenExpectedFiles.add(expected_file);
    ...
    if (rowErrors.length) errors.push(`manifest suite ${expected_file || index + 1}: ${rowErrors.join('; ')}`);
...
function rowOutcomeSnippet(row: any): string {
...
function validatePositiveOutcomeProof(rows: any[], artifactText: string, label: string): string[] {
  const errors: string[] = [];
  rows.forEach((row: any) => {
    ...
    if (!String(row?.scored_by || row?.reviewer || row?.judge || '').trim()) errors.push(`metric_derivation ${label} row ${id} scorer/reviewer is required`);
    const snippet = rowOutcomeSnippet(row);
    if (!snippet) errors.push(`metric_derivation ${label} row ${id} support_snippet is required`);
    else if (!artifactText.includes(snippet)) errors.push(`metric_derivation ${label} row ${id} support_snippet is absent from provenance artifact`);
  });
  return errors;
}
...
function validateGoldenMetricDerivation(root: string, parsed: any, expected: any): string[] {
  ...
  if (expectedFacts.length === 0) errors.push('expected facts are required for golden metric derivation');
  if (rows.length === 0) errors.push('golden fact rows are required for metric derivation');

  const expectedById = new Map<string, any>();
  for (const fact of expectedFacts) {
    const id = String(fact?.id || '').trim();
    if (!id) errors.push('expected fact id is required for golden metric derivation');
    else expectedById.set(id, fact);
  }
  ...
  for (const row of rows) {
    ...
    if (seen.has(id)) errors.push(`golden fact row ${id} is duplicated`);
    seen.add(id);
    ...
    if (String(row?.expected || '').trim() !== expectedText) errors.push(`golden fact row ${id} expected text must match expected fact`);
    if (row?.found === true) {
      const matched = String(row?.matched_text || '').trim();
      if (!matched) errors.push(`golden fact row ${id} matched_text is required when found`);
      else if (expectedText && !matched.includes(expectedText)) errors.push(`golden fact row ${id} matched_text must include expected fact text`);
      if (!String(row?.source || '').trim()) errors.push(`golden fact row ${id} source is required when found`);
      if (expectedFact.evidence_required === true && row?.evidence_present !== true) errors.push(`golden fact row ${id} evidence_present is required`);
    }
  }
  for (const id of expectedById.keys()) {
    if (!seen.has(id)) errors.push(`golden fact row ${id} is missing`);
  }
  ...
  for (const key of requiredCounts) {
    if (!finiteNumber(derivation[key])) errors.push(`metric_derivation.${key} must be a finite number`);
    else if (derivation[key] < 0 || Math.floor(derivation[key]) !== derivation[key]) errors.push(`metric_derivation.${key} must be a non-negative integer`);
  }

  const foundRows = rows.filter((row: any) => row?.found === true);
  const evidenceRows = foundRows.filter((row: any) => row?.evidence_present === true);
  ...
  if (finiteNumber(derivation.expected_fact_count) && derivation.expected_fact_count !== expectedFacts.length) errors.push('metric_derivation.expected_fact_count must match expected facts');
  if (finiteNumber(derivation.found_fact_count) && derivation.found_fact_count !== foundRows.length) errors.push('metric_derivation.found_fact_count must match golden fact rows');
  if (finiteNumber(derivation.evidence_backed_found_fact_count) && derivation.evidence_backed_found_fact_count !== evidenceRows.length) errors.push('metric_derivation.evidence_backed_found_fact_count must match golden fact rows');
  ...
  if (expectedFacts.length > 0) {
    ...
    if (!numbersEqual(metrics.fact_recall, factRecall)) errors.push('metrics.fact_recall must match golden fact rows');
  }
  if (foundRows.length > 0) {
    const evidencePrecision = evidenceRows.length / foundRows.length;
    if (!numbersEqual(metrics.evidence_precision, evidencePrecision)) errors.push('metrics.evidence_precision must match golden fact rows');
  ...
  if (bundle) {
    for (const row of foundRows) {
      ...
      const hit = bundleFacts.find((fact: any) => fact.text === matched && fact.source === source);
      if (!hit) errors.push(`golden fact row ${id} matched_text/source must be present in suite bundle analysis_document`);
      else if (Boolean(hit.has_evidence) !== Boolean(row?.evidence_present)) errors.push(`golden fact row ${id} evidence_present must match suite bundle evidence`);
    }
  }
  return errors;
}
...
export function goldenProofStatus(root: string, analysis: string, expectedSourceCommit: string | null = gitCommit(root)): any {
  ...
  if (!aggregate) aggregateErrors.push('missing golden aggregate results.json');
    ...
    if (expectedSourceCommit && aggregate.source_commit !== expectedSourceCommit) {
      ...
      if (changed.length) aggregateErrors.push(`golden aggregate subject changed since source_commit: ${changed.slice(0, 8).join(', ')}`);
    }
    ...
    if (aggregateCoverage.ready !== representative.ready) aggregateErrors.push('golden aggregate representative_coverage.ready must reflect validated manifest coverage');
    if (Number(aggregateCoverage.distinct_repositories || 0) !== representative.distinct_repositories) aggregateErrors.push('golden aggregate representative_coverage.distinct_repositories must match validated manifest coverage');
    if (asList(aggregateCoverage.missing_categories).join('|') !== representative.missing_categories.join('|')) aggregateErrors.push('golden aggregate representative_coverage.missing_categories must match validated manifest coverage');
    if (aggregate.market_proof_ready !== (expectedFiles.length >= 5 && expected.every(item => item.valid) && representative.ready === true)) aggregateErrors.push('golden aggregate market_proof_ready must reflect validated artifacts and representative coverage');
  }
  ...
  const failures = [
    ...
    ...(representative.ready ? [] : [
      ...representative.errors.map((error: string) => `golden representative manifest: ${error}`),
