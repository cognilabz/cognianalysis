export function analysisPipelineArtifact(tasks: any[] = [], capabilityTemplates: any[] = []): any {
  const byOutput = new Map(tasks.map((task: any) => [String(task.expected_output || ''), task]));
  const optionalTemplates = capabilityTemplates.map((template: any) => ({
  ...
  return {
    ...
    stages: [
      ...
      },
      {
        ...
        outputs: ['.analysis/report/index.html', '.analysis/report/analysis-data.json']
      }
    ]
  };
}
...
export function computeAnalysisPipelineContract(bundle: any): any {
  ...
  const skillMaterializationStage = stages.find((stage: any) => stage.id === 'deterministic_skill_workbench_task_materialization') || {};
  const skillReviewStage = stages.find((stage: any) => stage.id === 'llm_skill_workbench_reviews') || {};
