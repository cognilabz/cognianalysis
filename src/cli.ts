...
function runArtifactHashWorker(artifactPath: string, workerId: string, taskId: string): Promise<any> {
  const script = `
...
async function cmdProveOrchestration(args: string[]): Promise<number> {
  ...
  const completedTasks = (bundle.source_tier_backlog?.rows || [])
    ...
    .map((row: any) => {
      const task = (bundle.source_tier_task_manifest?.tasks || [])
        .find((item: any) => String(item?.id || '') === String(row?.id || ''));
      const expectedOutput = String(task?.expected_output || row?.expected_output || '').trim();
      return {
        id: String(row?.id || task?.id || '').trim(),
        expected_output: expectedOutput,
        artifact_path: Path.join(analysis, expectedOutput),
        artifact_hash: hashes.get(expectedOutput) || ''
      };
    })
    .filter((task: any) => task.id && task.expected_output && task.artifact_hash && FS.existsSync(task.artifact_path));
  ...
  const workerTasks = await Promise.all(selected.map((task: any, index: number) =>
    runArtifactHashWorker(task.artifact_path, `worker-${index + 1}`, task.id)
  ));
  const parallelProof = {
    schemaVersion: '1.0',
    complete: true,
