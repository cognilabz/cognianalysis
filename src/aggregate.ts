...
function validateCacheLedger(bundle: any): any {
  ...
  const storeWorkpackReceiptsValid = hitEntries.every((entry: any) => {
    const path = String(entry?.artifact_path || entry?.path || '').trim();
    ...
    return workpackExecutions.some((execution: any) =>
      String(execution?.artifact_path || '').trim() === path
...
const REPORT_QUALITY_REVIEW_CHECKS = [
  ...
  { id: 'stakeholder_report_style', label: 'Report reads as stakeholder documentation, with code identifiers used as citations rather than the main narrative' },
  { id: 'clear_reader_categories', label: 'Reader categories are clear, outcome-oriented and understandable before technical drilldown' },
  { id: 'freeform_llm_authored_report', label: 'Primary visible report is a free-flow LLM-authored assessment narrative, not a component checklist' },
  { id: 'consulting_grade_narrative', label: 'Narrative explains what, why, how, risk and decision implications in a polished assessment voice' },
  { id: 'reader_comprehension_review', label: 'LLM performed a reader-comprehension review for readers unfamiliar with the repository' },
  { id: 'concrete_examples_and_implications', label: 'Important flows, risks and recommendations include concrete examples and implications' },
  { id: 'jargon_and_domain_terms_explained', label: 'Domain terms, acronyms and internal names are explained before they are relied on' },
