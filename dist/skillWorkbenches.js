...
exports.SKILL_WORKBENCH_VERSION = void 0;
exports.writeSkillWorkbenchTasksFromLlmStrategy = writeSkillWorkbenchTasksFromLlmStrategy;
const utils_1 = require("./utils");
exports.SKILL_WORKBENCH_VERSION = 'skill-workbench-v1';
function directStrategy(strategyDoc) {
...
function plannedSkillRows(strategyDoc) {
    const strategy = directStrategy(strategyDoc);
    return (0, utils_1.asList)(strategy?.skill_application_plan || strategy?.skills || strategy?.planned_skill_workbenches);
}
...
function writeSkillWorkbenchTasksFromLlmStrategy(analysisDir, strategyDoc) {
    ...
    const tasks = rows.map((row, index) => {
        ...
        (0, utils_1.writeText)(utils_1.Path.join(tasksDir, filename), skillWorkbenchTaskBody(task, row));
        return task;
    });
    const manifest = {
        mode: 'llm_strategy_skill_workbenches',
        version: exports.SKILL_WORKBENCH_VERSION,
    ...
    (0, utils_1.writeJson)(utils_1.Path.join(analysisDir, 'skill-workbench-task-manifest.json'), manifest);
    (0, utils_1.writeJson)(utils_1.Path.join(analysisDir, 'data', 'skill-workbench-task-manifest.json'), manifest);
    return manifest;
}
function skillWorkbenchTaskBody(task, originalRow) {
