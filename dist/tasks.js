...
function writeLlmTasks(analysisDir, codeMap) {
    ...
    (0, utils_1.writeText)(utils_2.Path.join(analysisDir, 'llm_instructions.md'), overview(profile, modules, signals, glossary, capsules, artifactCandidates, componentLibrary, skillCatalog, goalContract, toolPositioningReferences));
    ...
    const pipeline = (0, analysisPipeline_1.analysisPipelineArtifact)(taskDefs);
    ...
    (0, utils_1.writeJson)(utils_2.Path.join(analysisDir, 'task-manifest.json'), { mode: 'llm_first', implementation_language: 'TypeScript', pipeline, tasks: taskDefs });
    return taskDefs;
}
...
function overview(profile, modules, signals, glossary, capsules, importantDocs, componentLibrary, skillCatalog, goalContract, toolPositioningReferences) {
...
function taskBody(task, profile, modules, signals, capsules, glossary, importantDocs, componentLibrary, skillCatalog, goalContract, toolPositioningReferences) {
    const hints = {
        repo: profile,
        source_inventory: {
