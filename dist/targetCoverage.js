...
function hasSemanticSource(bundle, key) {
    const presence = bundle.llm_output_presence || {};
    const detailReviews = (0, utils_1.asList)(bundle.source_family_detail_reviews);
    if (key === 'tasks' || key === 'signals')
        return true;
    if (key.startsWith('tooling.') || key.startsWith('report_artifacts.'))
        return true;
    if (key === 'source_inventory_accounting.complete' || key === 'source_coverage.complete')
        return !!(presence.analysis_coverage || detailReviews.length);
    if (key === 'analysis_coverage')
        return !!(presence.analysis_coverage || detailReviews.length);
    if (key === 'evidence_index')
        return (0, utils_1.asList)(bundle.evidence_index).length > 0;
    if (key === 'source_family_detail_reviews')
        return detailReviews.length > 0;
    if (key === 'detail_task_manifest.tasks')
        return !!(presence.detail_agent_plan || presence['detail_agent_plan.tasks']);
    if (key.startsWith('llm_detail_agent_plan') || key.startsWith('source_family_detail_review_coverage'))
        return !!(presence.detail_agent_plan || presence['detail_agent_plan.tasks']);
    if (key.startsWith('analysis_goal_trace_alignment'))
        return !!presence['analysis_document.requirements_trace'];
    if (key.startsWith('analysis_document') || key.startsWith('report_mode'))
        return !!presence.analysis_document;
...
function computeTargetCoverage(bundle) {
    return exports.TARGET_CAPABILITIES.map(cap => {
        ...
        for (const key of cap.output_keys) {
            ...
            else if (key === 'source_inventory_accounting.complete')
                present = (bundle.source_inventory_accounting || bundle.source_coverage)?.complete === true;
            else if (key === 'source_coverage.complete')
                present = bundle.source_coverage?.complete === true;
            else
                present = hasContent(getByPath(bundle, key));
            if (present && !hasSemanticSource(bundle, key))
                present = false;
            if (present)
                hits++;
