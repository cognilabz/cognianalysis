...
function renderReport(analysisDir, outputDir, title) {
    ...
    const dataPath = utils_1.Path.join(out, 'analysis-data.json');
    (0, utils_1.writeJson)(dataPath, bundle);
    (0, utils_1.writeJson)(utils_1.Path.join(analysisDir, 'data', 'report-artifacts.json'), {
        output_dir: out,
...
function chips(values) {
    ...
    return values.slice(0, 28).map((v) => chip(v)).join('');
}
...
function evidenceHtml(items) {
    ...
    const body = items.slice(0, 16).map((ev) => {
        ...
        return `<div class="evidence-item ${ok ? 'ok' : 'bad'}" id="${id}"><div class="path">${(0, utils_1.escapeHtml)(label)}</div>${ev?.symbol ? `<div class="small muted">${(0, utils_1.escapeHtml)(ev.symbol)}</div>` : ''}${ev?.reason ? `<div class="small bad-text">${(0, utils_1.escapeHtml)(ev.reason)}</div>` : ''}${ev?.snippet ? `<pre>${(0, utils_1.escapeHtml)(ev.snippet)}</pre>` : ''}</div>`;
    }).join('');
    const links = items.slice(0, 8).map((ev, index) => {
        const target = `evidence-${evidenceRenderCounter - Math.min(items.length, 16) + index + 1}`;
        return `<a class="evidence-link" href="#${target}" data-evidence-target="${target}">${(0, utils_1.escapeHtml)(evidenceLabel(ev))}</a>`;
    }).join('');
...
function paragraphs(values) {
    ...
    return items
        .map((v) => typeof v === 'string' ? v : firstText(v?.business_need, v?.business_use, v?.technical_drilldown, v?.summary, v?.description, v?.rationale, v?.recommendation, v?.gap, v?.reason))
        .filter(Boolean)
        .slice(0, 10)
        .map((v) => `<p>${(0, utils_1.escapeHtml)(v)}</p>`)
        .join('');
}
...
function statementHasDisplayContent(x) {
    return [
        x?.title,
        x?.name,
        x?.criterion,
...
function statementList(items) {
    ...
    return listItems(displayItems, (x) => {
        ...
        return `<div class="statement"><strong>${(0, utils_1.escapeHtml)(title)}</strong>${x.confidence ? ` ${confidenceChip(x.confidence)}` : ''}${x.severity ? ` ${riskChip(x.severity)}` : ''}${x.owner ? ` ${chip(x.owner)}` : ''}${text ? `<p>${(0, utils_1.escapeHtml)(text)}</p>` : ''}${evidenceHtml(evidenceOf(x))}</div>`;
    });
}
function mermaidBlock(source, evidence) {
    const text = typeof source === 'string' ? source : firstText(source?.source, source?.mermaid);
    if (!text)
        return '';
    return `<div class="mermaid-box" data-mermaid-box><div class="mermaid-output muted small">Rendering Mermaid diagram...</div><details><summary>Mermaid source</summary><pre class="mermaid-source">${(0, utils_1.escapeHtml)(text)}</pre></details>${evidenceHtml(evidence || evidenceOf(source))}</div>`;
}
...
function labelFor(block, key, fallback) {
    return firstText(block?.labels?.[key], block?.[`${key}_label`], fallback) || fallback;
}
function tableHeader(labels) {
    return `<thead><tr>${labels.map(label => `<th>${(0, utils_1.escapeHtml)(label)}</th>`).join('')}</tr></thead>`;
}
function renderDocMetricGrid(block) {
    const metrics = listItems(block.metrics || [], (m) => metric(m.label || labelFor(block, 'metric', 'Metric'), m.value ?? '', m.detail || ''));
    return `${metrics ? `<div class="metrics compact">${metrics}</div>` : ''}${evidenceHtml(evidenceOf(block))}`;
}
function renderSourceFamilyMap(block) {
    const families = listItems(block.families || [], (f) => `<article class="family-card search-card" data-search="${(0, utils_1.escapeHtml)(`${f.name || ''} ${f.role || ''} ${f.business_use || ''} ${f.technical_shape || ''}`)}">
    <div class="family-head"><h3>${(0, utils_1.escapeHtml)(f.name || labelFor(block, 'family', 'Source family'))}</h3>${f.confidence ? confidenceChip(f.confidence) : chip(f.evidence_level || labelFor(block, 'family_status', 'source family'))}</div>
...
function renderBoundaryMap(block) {
...
function renderDocFlow(block) {
  ...
  ${block.steps?.length ? `<div class="doc-steps">${listItems(block.steps, (s) => `<div class="step"><span>${(0, utils_1.escapeHtml)(s.order || '')}</span><div><strong>${(0, utils_1.escapeHtml)(s.actor || s.kind || 'step')}</strong><p>${(0, utils_1.escapeHtml)(s.description || '')}</p>${evidenceHtml(evidenceOf(s))}</div></div>`)}</div>` : ''}
  ${evidenceHtml(evidenceOf(block))}`;
}
function renderFourLevelAssessment(block) {
    const levels = listItems(block.levels || [], (l) => `<article class="level-card search-card" data-search="${(0, utils_1.escapeHtml)(`${l.level || ''} ${l.status || ''} ${l.summary || ''}`)}">
    <div class="family-head"><h3>${(0, utils_1.escapeHtml)(String(l.level || labelFor(block, 'level', 'level')).replace(/_/g, ' '))}</h3>${statusChip(l.status)}</div>
    ${paragraphs(l.summary)}
    ${l.next_steps?.length ? `<h4>${(0, utils_1.escapeHtml)(labelFor(block, 'next_steps', 'Next steps'))}</h4>${chips(l.next_steps)}` : ''}
    ${evidenceHtml(evidenceOf(l))}
  </article>`);
    return `${levels ? `<div class="level-grid">${levels}</div>` : ''}${evidenceHtml(evidenceOf(block))}`;
}
function renderDecisionMatrix(block) {
    const rows = (block.rows || []).map((r) => `<tr class="search-card" data-search="${(0, utils_1.escapeHtml)(`${r.decision || ''} ${r.recommendation || ''} ${r.risk || ''}`)}">
    <td><strong>${(0, utils_1.escapeHtml)(r.decision || labelFor(block, 'decision', 'Decision'))}</strong><div>${r.confidence ? confidenceChip(r.confidence) : ''}</div></td>
    <td>${chips(r.options || [])}</td>
    <td>${(0, utils_1.escapeHtml)(r.recommendation || '')}</td>
...
function renderRoadmap(block) {
    const items = listItems(block.items || [], (r) => `<article class="roadmap-item search-card" data-search="${(0, utils_1.escapeHtml)(`${r.title || ''} ${r.phase || ''} ${r.benefit || ''}`)}">
    <div class="family-head"><h3>${(0, utils_1.escapeHtml)(r.title || labelFor(block, 'item', 'Roadmap item'))}</h3><div>${r.phase ? chip(r.phase) : ''}${r.effort ? chip(r.effort) : ''}${r.risk ? riskChip(r.risk) : ''}</div></div>
...
function renderAgentPlan(block) {
    ...
    const rows = tasks.slice(0, 80).map((t) => {
        ...
        return `<tr class="search-card" data-search="${(0, utils_1.escapeHtml)(`${t.source_family || ''} ${t.recommended_agent || ''} ${(t.focus || []).join(' ')}`)}">
      <td><strong>${(0, utils_1.escapeHtml)(t.source_family || labelFor(block, 'source_family', 'source family'))}</strong><div class="small muted">${(0, utils_1.escapeHtml)(t.recommended_agent || '')}</div></td>
      <td>${stateChip}</td>
      <td>${chips(t.focus || [])}</td>
      <td>${chips(t.expected_outputs || [])}</td>
      <td>${t.task_file ? `<code>${(0, utils_1.escapeHtml)(t.task_file)}</code>` : `<span class="muted small">${(0, utils_1.escapeHtml)(labelFor(block, 'planned_status', 'planned'))}</span>`}${t.expected_output ? `<div class="small muted">${(0, utils_1.escapeHtml)(t.expected_output)}</div>` : ''}</td>
      <td>${chips((t.seed_files || []).map((f) => typeof f === 'string' ? f : f.path).filter(Boolean).slice(0, 4))}</td>
    </tr>`;
    }).join('');
...
function renderTechnicalDrilldown(block) {
    const refs = listItems(block.references || [], (r) => `<a class="jump-card search-card" href="${(0, utils_1.escapeHtml)(r.target || '#technical')}" data-search="${(0, utils_1.escapeHtml)(`${r.label || ''} ${r.description || ''}`)}"><strong>${(0, utils_1.escapeHtml)(r.label || labelFor(block, 'reference', 'Drilldown'))}</strong><p>${(0, utils_1.escapeHtml)(r.description || '')}</p></a>`);
    return `${refs ? `<div class="grid three">${refs}</div>` : ''}${evidenceHtml(evidenceOf(block))}`;
}
function renderOpenQuestions(block) {
    return statementList((block.items || []).map((q) => ({
...
function renderDocBlock(block) {
    const title = block.title ? `<h3>${(0, utils_1.escapeHtml)(block.title)}</h3>` : '';
    let body = '';
    switch (block.type) {
        case 'metric_grid':
            body = renderDocMetricGrid(block);
            break;
        case 'source_family_map':
            body = renderSourceFamilyMap(block);
            break;
        case 'boundary_map':
            body = renderBoundaryMap(block);
            break;
        case 'flow':
            body = renderDocFlow(block);
            break;
        case 'four_level_assessment':
            body = renderFourLevelAssessment(block);
            break;
        case 'decision_matrix':
            body = renderDecisionMatrix(block);
            break;
        case 'roadmap':
            body = renderRoadmap(block);
            break;
        case 'agent_plan':
            body = renderAgentPlan(block);
            break;
        case 'technical_drilldown':
            body = renderTechnicalDrilldown(block);
            break;
        case 'open_questions':
            body = renderOpenQuestions(block);
            break;
        case 'statement_list':
            body = statementList(block.items || []) + evidenceHtml(evidenceOf(block));
            break;
        case 'narrative':
        default:
            body = paragraphs(block.text || block.paragraphs || block.summary || block.description || block.business_need || block.business_use || block.technical_drilldown) + evidenceHtml(evidenceOf(block));
            break;
    }
    if (!title && !body)
        return '';
    return `<div class="doc-block ${(0, utils_1.escapeHtml)(block.type || 'narrative')}">${title}${body}</div>`;
}
function analysisDocumentSections(bundle) {
    ...
    for (const section of doc.sections) {
      ...
      <p class="doc-section-kicker">${(0, utils_1.escapeHtml)(section.level || 'analysis')}</p>
      ${section.intent ? `<p class="section-intent">${(0, utils_1.escapeHtml)(section.intent)}</p>` : ''}
      ${(section.blocks || []).map((block) => renderDocBlock(block)).join('')}
...
function pendingAnalysisDocumentSections(bundle) {
    ...
    <h2>Report pending · ${(0, utils_1.escapeHtml)(bundle.profile?.repo_name || 'Repository')}</h2>
...
function buildHtml(bundle, title) {
    ...
    const heroEyebrow = hasAuthoredReport
        ...
        : 'LLM-authored report pending';
    const sections = authoredSections.length ? [
        ...authoredSections
    ] : [
