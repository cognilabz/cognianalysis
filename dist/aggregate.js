...
function aggregate(repo, analysisDir) {
    ...
    (0, utils_1.writeJson)(utils_1.Path.join(dataDir, 'product-artifact-model.json'), bundle.product_artifact_model);
    (0, utils_1.writeJson)(utils_1.Path.join(dataDir, 'simplified-harness-contract.json'), bundle.simplified_harness_contract);
    (0, utils_1.writeJson)(utils_1.Path.join(dataDir, 'parallel-orchestration-contract.json'), bundle.parallel_orchestration_contract);
...
function commandSet(value) {
    return new Set((0, utils_1.asList)(value).map((item) => String(item || '').trim()).filter(Boolean));
}
function missingCommands(actual, expected) {
    return expected.filter(command => !actual.has(command));
}
function computeProductArtifactModel(bundle) {
    ...
    const checks = [
        ...
        { id: 'task_guide_model', ready: (0, utils_1.asList)(bundle.tasks).length > 0 && bundle.analysis_pipeline?.pipeline_kind === 'llm_driven_overview_detail_final_report', evidence: `tasks=${(0, utils_1.asList)(bundle.tasks).length}` },
    ...
    return {
        ...
        hidden_or_dev_scoped_commands: (0, utils_1.asList)(bundle.tooling?.internal_cli_commands),
        compatibility_aliases: (0, utils_1.asList)(bundle.tooling?.compatibility_cli_commands),
...
function computeSimplifiedHarnessContract(bundle) {
    const publicCommands = commandSet(bundle.tooling?.public_cli_commands);
    const internalCommands = (0, utils_1.asList)(bundle.tooling?.internal_cli_commands).map((item) => String(item || ''));
    const compatibilityCommands = (0, utils_1.asList)(bundle.tooling?.compatibility_cli_commands).map((item) => String(item || ''));
    const requiredPublicCommands = ['analyze', 'status', 'open', 'eval'];
    const checks = [
        {
        ...
        },
        {
            ...
            ready: (0, utils_1.asList)(bundle.tasks).length > 0 && bundle.analysis_pipeline_contract?.complete === true,
            evidence: `tasks=${(0, utils_1.asList)(bundle.tasks).length}, pipeline=${bundle.analysis_pipeline_contract?.complete === true}`
        },
        {
...
function artifactHashSet(bundle) {
    return new Set((0, utils_1.asList)(bundle.artifact_dependency_graph?.nodes)
...
function sourceTierWorkpackTaskContextHash(task, receipt = {}) {
    return hashStable({
...
function fileContentHash20(file) {
    try {
        return (0, utils_1.sha1Short)(utils_1.FS.readFileSync(file, 'utf8'), 20);
    }
    catch {
        return '';
    }
...
function validateParallelExecutionProof(bundle) {
    ...
    const logTasks = (0, utils_1.asList)(bundle.orchestration_execution_log?.worker_tasks || bundle.orchestration_execution_log?.tasks);
    const generatedFrom = (0, utils_1.asList)(proof.generated_from).map(String);
    const workerIds = new Set(tasks.map((task) => String(task?.worker_id || '').trim()).filter(Boolean));
    const taskIds = new Set(tasks.map((task) => String(task?.task_id || '').trim()).filter(Boolean));
    const missing = [
    ...
    const proofTasksMatchLog = tasks.length > 0 && tasks.every((task) => {
        ...
        const artifactHash = String(task?.artifact_hash || task?.output_hash || '').trim();
        return logTasks.some((row) => String(row?.worker_id || '').trim() === workerId
...
function validateCacheLedger(bundle) {
    ...
    const workpackExecutions = (0, utils_1.asList)(bundle.source_tier_workpack_executions).filter((execution) => execution?.valid === true);
    const cacheStoreByKey = new Map((0, utils_1.asList)(bundle.cache_store_entries)
        .map((entry) => [String(entry?.cache_key || '').trim(), entry])
        .filter((entry) => entry[0]));
    const missing = [
    ...
    const storeProvenanceValid = hitEntries.every((entry) => {
    ...
    if (!storeProvenanceValid)
        missing.push('cache_ledger.cache_store_provenance');
    const storeWorkpackReceiptsValid = hitEntries.every((entry) => {
        const path = String(entry?.artifact_path || entry?.path || '').trim();
        const taskId = String(entry?.task_id || '').trim();
        const taskContextHash = String(entry?.task_context_hash || '').trim();
        const receiptHash = String(entry?.execution_receipt_hash || '').trim();
        return workpackExecutions.some((execution) => String(execution?.artifact_path || '').trim() === path
            && String(execution?.task_id || '').trim() === taskId
            && String(execution?.task_context_hash || '').trim() === taskContextHash
            && String(execution?.receipt_hash || '').trim() === receiptHash);
    });
    if (!storeWorkpackReceiptsValid)
        missing.push('cache_ledger.cache_store_workpack_receipts');
    const storeFilesValid = hitEntries.every((entry) => {
        const key = String(entry?.cache_key || entry?.key || '').trim();
        ...
        return store?.schemaVersion === '1.0'
            ...
            && String(store?.cache_store_path || '').trim() === utils_1.Path.join('cache', 'source-tier', `${key}.json`).replace(/\\/g, '/');
    });
    if (!storeFilesValid)
        missing.push('cache_ledger.cache_store_files');
...
function validateCacheReuseProof(bundle) {
    ...
    const ledgerEntries = (0, utils_1.asList)(bundle.cache_ledger?.cache_entries || bundle.cache_ledger?.entries);
    const generatedFrom = (0, utils_1.asList)(proof.generated_from).map(String);
    const hitEntries = entries.filter((entry) => entry?.hit === true || entry?.cache_hit === true);
    const requestHash = String(bundle.product_analysis_request?.request_hash || '').trim();
    const missing = [
    ...
    const proofEntriesMatchLedger = hitEntries.length > 0 && hitEntries.every((entry) => {
        const key = String(entry?.cache_key || entry?.key || '').trim();
        const path = String(entry?.artifact_path || entry?.path || '').trim();
        const hash = String(entry?.artifact_hash || entry?.source_hash || entry?.content_hash || '').trim();
        return ledgerEntries.some((row) => (row?.hit === true || row?.cache_hit === true)
    ...
    if (!proofEntriesMatchLedger)
        missing.push('cache_entries_match_cache_ledger');
    return {
        valid: missing.length === 0,
        missing,
...
function computeParallelOrchestrationContract(bundle) {
    ...
    const scaffoldChecks = [
        ...
        },
        {
            ...
            ready: graph.complete === true && (0, utils_1.asList)(graph.nodes).some((node) => String(node?.content_hash || '').trim()),
            evidence: `graph=${graph.complete === true}, nodes=${(0, utils_1.asList)(graph.nodes).length}`
        },
        {
            id: 'run_provenance_available',
            ready: provenance.complete === true && (0, utils_1.asList)(provenance.generated_from_matrix).length > 0,
            evidence: `provenance=${provenance.complete === true}, artifacts=${(0, utils_1.asList)(provenance.generated_from_matrix).length}`
        }
    ];
    const proofChecks = [
        {
...
function evidenceLineSortValue(ev) {
...
function loadSourceTierOutputs(sourceTierDir) {
    ...
    const analysisDir = utils_1.Path.dirname(sourceTierDir);
    ...
    for (const f of files) {
        ...
        if (review && receipt && typeof receipt === 'object' && !Array.isArray(receipt)) {
            const generatedFrom = (0, utils_1.asList)(receipt.generated_from).map((path) => String(path || '').trim()).filter(Boolean);
            const taskFile = generatedFrom.find((path) => path.startsWith('source_tier_tasks/') && path.endsWith('.md')) || '';
            const taskFileHash = taskFile ? fileContentHash20(utils_1.Path.join(analysisDir, taskFile)) : '';
            const contextFile = `source_tier_contexts/${String(receipt.task_id || review.task_id || utils_1.Path.basename(f, '.json')).trim()}.json`;
            const contextFileHash = utils_1.FS.existsSync(utils_1.Path.join(analysisDir, contextFile)) ? fileContentHash20(utils_1.Path.join(analysisDir, contextFile)) : '';
...
function loadCacheStoreEntries(analysisDir) {
    const cacheDir = utils_1.Path.join(analysisDir, 'cache', 'source-tier');
    if (!utils_1.FS.existsSync(cacheDir))
        return [];
    return utils_1.FS.readdirSync(cacheDir)
        .filter((name) => name.endsWith('.json'))
        .sort()
        .map((name) => ({
        ...(0, utils_1.loadJson)(utils_1.Path.join(cacheDir, name), {}),
        cache_store_path: utils_1.Path.join('cache', 'source-tier', name).replace(/\\/g, '/')
    }));
}
...
function validateEvidence(repo, ev) {
    ...
    if (fullReal !== repoRootReal && !fullReal.startsWith(`${repoRootReal}${utils_1.Path.sep}`))
        return { ...ev, line, valid: false, reason: 'path escapes repository' };
