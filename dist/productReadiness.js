...
function traceStatus(bundle, needle) {
    ...
    const row = rows.find((item) => String(item.label || '').toLowerCase().includes(lower));
...
function hasEvidenceBackedItems(bundle, key) {
    return asList(bundle?.[key]).some((item) => asList(item?.evidence).length > 0);
}
...
function computeProductReadiness(repo, analysis, bundle, marketProof) {
    ...
    const examplesReady = hasEvidenceBackedItems(bundle, 'interfaces')
        || asList(bundle?.documentation?.request_response_examples).some((item) => asList(item?.evidence).length > 0);
    ...
    const checks = [
        ...
        check('evidence_backed', 'Visible claims are evidence-backed', evidenceReady, `unsupported=${Number(bundle?.analysis_document_report_lint?.unsupported_claim_count || 0)}, invalid_evidence=${asList(bundle?.evidence_index).filter((item) => item?.valid === false).length}`, 'Resolve unsupported claims and invalid evidence references.'),
